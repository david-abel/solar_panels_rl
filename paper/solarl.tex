...
Also of interest is the {\it action-value function}, $Q^*: \mc{S} \times \mc{A} \longmapsto \left[0,\frac{\textsc{RMax}}{1-\gamma}\right]]$, which specifies the long term expected reward of executing an action in a state and behaving optimally thereafter:
\begin{equation}
...
\end{equation}
where $\eta \in [0,1]$ is a learning rate. The linear approximator extends tabular $Q$-Learning to domains where states are described by feature vectors, $\bm{s} = \begin{bmatrix} s_1& s_2& \ldots& s_k\end{bmatrix}$. Here, $\hat{Q}$ is parameterized by a set of $k$-vectors $w^a$, where each vector corresponds to action $a$'s parameters across the state variables, resulting in:
\begin{equation}
