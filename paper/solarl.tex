...
In this work, we advocate for the use the computational paradigm of Reinforcement Learning (RL) to optimize solar panel performance. Using RL, a learned solar panel controller can account for weather change, cloud coverage, diverse reflective indices of surroundings, and changing atmospheric conditions, offering an efficient yet adaptive solution that can optimize for the given availability of each type of solar irradiance, without the need for complex hardware. Our primary contribution is twofold: (1) the advancement of an important application area as an RL benchmark with a high fidelity simulation based on recent models of solar irradiance, and (2) we validate the utility of the AI paradigm of RL for solar tracking in our simulation.
