...
# Local imports.
from simple_rl.tasks import ChainMDP, GridWorldMDP, TaxiOOMDP, SolarMDP, SolarOOMDP, SolarStationaryOOMDP
...
def choose_mdp(mdp_name, atari_game="centipede"):
    '''
    Args:
        mdp_name (str): one of {atari, grid, chain, taxi, solar}
    ...
    # solar_mdp = SolarStationaryOOMDP()

    # Taxi MDP.
    agent = {"x":1, "y":1, "has_passenger":0}
    passengers = [{"x":4, "y":3, "dest_x":7, "dest_y":2, "in_taxi":0}]
    walls = []
    taxi_mdp = TaxiOOMDP(10, 10, agent_loc=agent, walls=walls, passengers=passengers)
    if mdp_name == "atari":
        ...
        return {"grid":grid_mdp, "chain":chain_mdp, "taxi":taxi_mdp, "solar":solar_mdp}[mdp_name]
